...
def stock_list(request):
    ...
    predicted_dates = [date.strftime('%d-%m-%Y') for date in predicted_data['Date']]
    predicted_close_prices = predicted_data['Predicted_Close'].tolist()

    # Data Prediksi 
    predicted_dates = predicted_data['Date'].iloc[0].strftime('%d/%B/%Y') 
    predicted_close_prices = round(predicted_data['Predicted_Close'].iloc[0], 2)
